...
public:
    ...
    Street& operator=(std::unique_ptr<Street> other) {
        if (this != other.get()) {  // Self-assignment check
            ...
            this->setPrice(other->getPrice());   // Assuming Tradeable has setPrice()
            this->setRent(other->getRent());     // Assuming Tradeable has setRent()
            this->setName(other->getName());     // Assuming Square has setName()
            this->setType(other->getType());     // Assuming Tradeable has setType()
        }
        return *this;
    }
